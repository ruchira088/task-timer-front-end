...
A stopwatch web app, deployed to [timer.ruchij.com](https://timer.ruchij.com). Stamped from [`ruchira088/react-template`](https://github.com/ruchira088/react-template): React 19 + React Router 8 (SPA, no SSR) on Node 24, Vite 8, Vitest 4, TypeScript 7, oxlint, Tailwind CSS v4 + shadcn/ui, Zod 4, Sentry, Ansible playbooks for build artefacts and Docker, multi-stage GitHub Actions pipeline, and CDK deployment via the `react-app-cdk-deploy` library. The template's authentication and HTTP layers have been stripped out — see [No backend](#no-backend).
...
- Layout responds to **container queries** — `HomePage.module.scss` declares `container-type: inline-size` and `Timer.module.scss` scales the display against the container, not the viewport.
...
- **Logo, loader & favicon**: `app/images/small-logo.svg` and `app/images/loading.svg` are the template's generic slate placeholders. (`public/favicon.ico` is the app's own.)
...
There is no API and no sign-in. The timer runs entirely in the browser and keeps no server state, so the template's authentication layer — login/signup pages, `AuthenticationService`, the axios `HttpClient`, the mock API adapter, `ApiConfiguration` and the `User`/`AuthenticationToken` models — has been removed along with the `axios` dependency. What survives from the template's service layer is client-only: `Config.ts` (hostname → environment, for Sentry), `config/` + `kv-store/` (localStorage-backed theme persistence) and `Sentry.ts`.

If a backend is added later, pull those files back from [`ruchira088/react-template`](https://github.com/ruchira088/react-template) rather than rewriting them.
...
app/
  ...
  pages/AppLayout.tsx                  header (logo + theme toggle) wrapping the routes
